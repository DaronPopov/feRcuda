...
std::string detected_compute_arch() {
    int dev = 0;
    cudaGetDevice(&dev);
    cudaDeviceProp prop{};
    if (cudaGetDeviceProperties(&prop, dev) == cudaSuccess) {
...
std::string make_key(const fer_jit_source_t* source, const fer_jit_options_t* options) {
    ...
    // Always include the detected GPU arch in the key so cache entries
    // from different GPUs don't collide.
    oss << ":gpu" << detected_compute_arch();
    return oss.str();
}
...
constexpr uint64_t kDefaultMaxDiskCacheBytes = 256ull * 1024ull * 1024ull;  // 256 MiB
...
void scan_disk_cache(const char* dir, std::vector<DiskCacheEntry>* entries) {
    ...
    while ((ent = readdir(d)) != nullptr) {
        ...
        if (name.size() < 4 || name.substr(name.size() - 4) != ".ptx") continue;
        const std::string path = std::string(dir) + "/" + name;
        struct stat st{};
        if (stat(path.c_str(), &st) == 0 && S_ISREG(st.st_mode)) {
            entries->push_back(DiskCacheEntry{path, static_cast<uint64_t>(st.st_size), st.st_mtime});
        }
    }
    closedir(d);
}

void evict_disk_cache(const char* dir, uint64_t max_bytes) {
    if (!dir) return;
    std::vector<DiskCacheEntry> entries;
    scan_disk_cache(dir, &entries);
    uint64_t total = 0;
    for (const auto& e : entries) total += e.size;
    if (total <= max_bytes) return;
    // Sort oldest first
    std::sort(entries.begin(), entries.end(),
              [](const DiskCacheEntry& a, const DiskCacheEntry& b) { return a.mtime < b.mtime; });
...
Status JitManager::compile(
    ...
    *out_program = nullptr;
    ...
    if (!disk_cache_path.empty() && !ptx_for_disk_cache.empty()) {
        ...
        evict_disk_cache(options ? options->cache_dir : nullptr, kDefaultMaxDiskCacheBytes);
    }
...
Status JitManager::get_stats(fer_jit_stats_t* out_stats) const {
    ...
    *out_stats = impl_->stats;
    out_stats->memory_cache_entries = static_cast<uint64_t>(impl_->cache.size());
    // Disk cache stats are populated from the default intent cache directory.
    // A more complete approach would track per-session cache dirs.
    std::vector<DiskCacheEntry> entries;
    scan_disk_cache("/tmp/fercuda_jit_cache_intent", &entries);
    scan_disk_cache("/tmp/fercuda_jit_cache_agent", &entries);
    uint64_t total_bytes = 0;
    for (const auto& e : entries) total_bytes += e.size;
    out_stats->disk_cache_entries = static_cast<uint64_t>(entries.size());
    out_stats->disk_cache_bytes = total_bytes;
    return Status::ok_status();
}
