...
#ifndef TEST_SCRIPT_CANCEL_MODULE_PATH
#define TEST_SCRIPT_CANCEL_MODULE_PATH ""
#endif
...
int main() {
    if (std::strlen(TEST_SCRIPT_CANCEL_MODULE_PATH) == 0) return fail("missing TEST_SCRIPT_CANCEL_MODULE_PATH");
    ...
    if (fer_script_load(rt, TEST_SCRIPT_CANCEL_MODULE_PATH).code != FER_STATUS_OK) {
        return fail("fer_script_load");
    }

    fer_script_run_options_t opts{};
    opts.timeout_ms = 10;
    char err[256];
    err[0] = '\0';
    fer_status_t st = fer_script_run_with_options(rt, &opts, "{}", err, sizeof(err));
    if (st.code == FER_STATUS_OK) return fail("expected timeout/cancel failure");
    if (err[0] == '\0') return fail("expected error message");
    ...
    std::printf("SCRIPT TIMEOUT TEST PASSED\n");
    return 0;
}
