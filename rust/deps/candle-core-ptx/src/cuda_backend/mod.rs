...
use cudarc::driver::{
    CudaSlice, CudaStream, DevicePtr, DevicePtrMut, DeviceRepr, LaunchArgs, LaunchConfig,
    PushKernelArg, SyncOnDrop, ValidAsZeroBits,
};
use half::{bf16, f16};
use std::sync::Arc;
...
static NULL_DEVICE_PTR: cudarc::driver::sys::CUdeviceptr = 0;

unsafe impl<'a, 'b: 'a, T> PushKernelArg<&'b SlicePtrOrNull<T>> for LaunchArgs<'a> {
    #[inline(always)]
    fn arg(&mut self, arg: &'b SlicePtrOrNull<T>) -> &mut Self {
        match arg {
            SlicePtrOrNull::Ptr(slice) => {
                <LaunchArgs<'a> as PushKernelArg<&CudaSlice<T>>>::arg(self, slice)
            }
            SlicePtrOrNull::Null => {
                <LaunchArgs<'a> as PushKernelArg<&cudarc::driver::sys::CUdeviceptr>>::arg(
                    self,
                    &NULL_DEVICE_PTR,
                )
            }
...
fn device_ptr_offset<'a, T>(
    ...
    (ptr + byte_offset as u64, sync)
}

fn device_ptr_mut_offset<'a, T>(
    slice: &'a mut CudaSlice<T>,
    offset: usize,
    stream: &'a CudaStream,
) -> (cudarc::driver::sys::CUdeviceptr, SyncOnDrop<'a>) {
    let (ptr, sync) = slice.device_ptr_mut(stream);
    let byte_offset = offset * std::mem::size_of::<T>();
    (ptr + byte_offset as u64, sync)
}
...
impl Map1 for Affine {
    fn f<T: DeviceRepr + WithDType>(
        ...
        let scale = T::from_f64(self.0);
        let bias = T::from_f64(self.1);
        let params = (&el, &dims_len, &ds, src, &out, &scale, &bias);
        // SAFETY: ffi.
        let stream = dev.launch_stream();
        dev.launch_on_stream(&stream, &func, cfg, params).w()?;
        Ok(out)
    }
}
...
impl Map1 for Elu {
    fn f<T: DeviceRepr + WithDType>(
        ...
        let params = (&el, &dims_len, &ds, &alpha, src, &out);
        // SAFETY: ffi.
        let stream = dev.launch_stream();
        dev.launch_on_stream(&stream, &func, cfg, params).w()?;
        Ok(out)
...
impl Map1 for Powf {
    fn f<T: DeviceRepr + WithDType>(
        ...
        let params = (&el, &dims_len, &ds, &pow, src, &out);
        // SAFETY: ffi.
        let stream = dev.launch_stream();
        dev.launch_on_stream(&stream, &func, cfg, params).w()?;
        Ok(out)
...
impl<U: UnaryOpT> Map1 for U {
    fn f<T: DeviceRepr + WithDType + ValidAsZeroBits>(
        ...
        let dims_len = dims.len();
        let params = (&el_count, &dims_len, &ds, src, &out);
        // SAFETY: ffi.
        let stream = dev.launch_stream();
        dev.launch_on_stream(&stream, &func, cfg, params).w()?;
        Ok(out)
...
impl Map1 for IndexSelect<'_> {
    fn f<T: DeviceRepr + WithDType + ValidAsZeroBits>(
        ...
        let ids_l = &self.1;
        let stream = dev.launch_stream();
        let (name, ids, _ids_sync) = match &self.0.slice {
            CudaStorageSlice::U32(slice) => {
                let (ptr, sync) = device_ptr_offset(slice, ids_l.start_offset(), &stream);
                ("is_u32", ptr, sync)
            }
...
impl Map1 for Gather<'_> {
    fn f<T: DeviceRepr + WithDType + ValidAsZeroBits>(
        ...
        let ids_l = &self.1;
        let dim = self.2;
        let (ids_o1, _ids_o2) = match ids_l.contiguous_offsets() {
            Some(o12) => o12,
            None => Err(crate::Error::RequiresContiguous { op: "gather" }.bt())?,
        };
        let stream = dev.launch_stream();
        let (name, ids, _ids_sync) = match &ids.slice {
            CudaStorageSlice::U32(slice) => {
                let (ptr, sync) = device_ptr_offset(slice, ids_o1, &stream);
                ("gather_u32", ptr, sync)
            }
            CudaStorageSlice::U8(slice) => {
                let (ptr, sync) = device_ptr_offset(slice, ids_o1, &stream);
...
impl Map1 for Col2Im1D {
    fn f<T: DeviceRepr + WithDType + ValidAsZeroBits>(
        ...
        let cfg = LaunchConfig::for_num_elems(dst_el as u32);
        let params = (
            &dst_el, &l_out, &l_in, &c_out, &k_size, &stride, col, &mut im,
        );
...
impl Map2 for WhereCond<'_> {
    fn f<T: DeviceRepr + WithDType + ValidAsZeroBits>(
        ...
        let ids_l = &self.1;
        let stream = dev.launch_stream();
        let (ids, name, _ids_sync) = match &self.0.slice {
            CudaStorageSlice::U8(slice) => {
                let (ptr, sync) = device_ptr_offset(slice, ids_l.start_offset(), &stream);
                (ptr, "where_u8", sync)
            }
        ...
        let out = unsafe { dev.alloc::<T>(el) }.w()?;
        let dims_len = dims.len();
        let params = (&el, &dims_len, &ds, &ids, t, f, &out);
        // SAFETY: ffi
        dev.launch_on_stream(&stream, &func, cfg, params).w()?;
        Ok(out)
    }
...
impl<U: crate::op::BinaryOpT> Map2 for U {
    fn f<T: DeviceRepr + WithDType + ValidAsZeroBits>(
        ...
        let dims_len = dims.len();
        let params = (&elem_count, &dims_len, &dims_and_strides, lhs, rhs, &out);
        // SAFETY: ffi
        let stream = dev.launch_stream();
        dev.launch_on_stream(&stream, &func, cfg, params).w()?;
        Ok(out)
...
impl BackendStorage for CudaStorage {
    ...
    fn to_dtype(&self, layout: &Layout, dtype: DType) -> Result<Self> {
        ...
        // is used.
        let (inp, _inp_sync) = match &self.slice {
            CudaStorageSlice::U8(inp) => device_ptr_offset(inp, start_o, &stream),
            CudaStorageSlice::U32(inp) => device_ptr_offset(inp, start_o, &stream),
            CudaStorageSlice::I64(inp) => device_ptr_offset(inp, start_o, &stream),
            CudaStorageSlice::BF16(inp) => device_ptr_offset(inp, start_o, &stream),
            CudaStorageSlice::F16(inp) => device_ptr_offset(inp, start_o, &stream),
            CudaStorageSlice::F32(inp) => device_ptr_offset(inp, start_o, &stream),
            CudaStorageSlice::F64(inp) => device_ptr_offset(inp, start_o, &stream),
        };
        ...
        let slice = match dtype {
            DType::U8 => {
                ...
                let params = (&el, &dims_len, &ds, &inp, &out);
                dev.launch_on_stream(&stream, &func, cfg, params).w()?;
    ...
    fn to_cpu_storage(&self) -> Result<CpuStorage> {
        match &self.slice {
            CudaStorageSlice::U8(slice) => {
                let cpu_storage = self.device().dtoh_sync_copy(slice).w()?;
    ...
    fn matmul(
        ...
        let dev = &self.device;
        let blas_stream = dev.blas_stream();
        let slice = match (&self.slice, &rhs.slice) {
            (CudaStorageSlice::BF16(lhs), CudaStorageSlice::BF16(rhs)) => {
                ...
                unsafe {
                    gemm_strided_batched_bf16(&self.device.blas, &blas_stream, cfg, rhs, lhs, &mut out)
                }
                    .w()?;
                CudaStorageSlice::BF16(out)
            }
            (CudaStorageSlice::F16(lhs), CudaStorageSlice::F16(rhs)) => {
                ...
                unsafe {
                    gemm_strided_batched_f16(&self.device.blas, &blas_stream, cfg, rhs, lhs, &mut out)
                }
                    .w()?;
                CudaStorageSlice::F16(out)
            }
            (CudaStorageSlice::F32(lhs), CudaStorageSlice::F32(rhs)) => {
                ...
                unsafe {
                    gemm_strided_batched_f32(&self.device.blas, &blas_stream, cfg, rhs, lhs, &mut out)
                }
                    .w()?;
                CudaStorageSlice::F32(out)
            }
    ...
    fn copy2d(
        ...
        let stream = dev.launch_stream();
        let (src, dst, _src_sync, _dst_sync, kname) = match (&self.slice, &mut dst.slice) {
            (S::U8(s), S::U8(d)) => {
                let (src_ptr, src_sync) = device_ptr_offset(s, src_o, &stream);
                let (dst_ptr, dst_sync) = device_ptr_mut_offset(d, dst_o, &stream);
                (src_ptr, dst_ptr, src_sync, dst_sync, "copy2d_u8")
            }
            (S::U32(s), S::U32(d)) => {
                let (src_ptr, src_sync) = device_ptr_offset(s, src_o, &stream);
                let (dst_ptr, dst_sync) = device_ptr_mut_offset(d, dst_o, &stream);
                (src_ptr, dst_ptr, src_sync, dst_sync, "copy2d_u32")
            }
            (S::I64(s), S::I64(d)) => {
                let (src_ptr, src_sync) = device_ptr_offset(s, src_o, &stream);
                let (dst_ptr, dst_sync) = device_ptr_mut_offset(d, dst_o, &stream);
                (src_ptr, dst_ptr, src_sync, dst_sync, "copy2d_i64")
            }
            (S::BF16(s), S::BF16(d)) => {
                let (src_ptr, src_sync) = device_ptr_offset(s, src_o, &stream);
                let (dst_ptr, dst_sync) = device_ptr_mut_offset(d, dst_o, &stream);
                (src_ptr, dst_ptr, src_sync, dst_sync, "copy2d_bf16")
            }
            (S::F16(s), S::F16(d)) => {
                let (src_ptr, src_sync) = device_ptr_offset(s, src_o, &stream);
                let (dst_ptr, dst_sync) = device_ptr_mut_offset(d, dst_o, &stream);
                (src_ptr, dst_ptr, src_sync, dst_sync, "copy2d_f16")
            }
            (S::F32(s), S::F32(d)) => {
                let (src_ptr, src_sync) = device_ptr_offset(s, src_o, &stream);
                let (dst_ptr, dst_sync) = device_ptr_mut_offset(d, dst_o, &stream);
                (src_ptr, dst_ptr, src_sync, dst_sync, "copy2d_f32")
            }
            (S::F64(s), S::F64(d)) => {
                let (src_ptr, src_sync) = device_ptr_offset(s, src_o, &stream);
                let (dst_ptr, dst_sync) = device_ptr_mut_offset(d, dst_o, &stream);
                (src_ptr, dst_ptr, src_sync, dst_sync, "copy2d_f64")
            }
        ...
        match (&self.slice, &mut dst.slice) {
            (CudaStorageSlice::BF16(src), CudaStorageSlice::BF16(dst)) => {
                ...
                if src_l.is_contiguous() {
                    ...
                    // SAFETY: Set later by running the kernel.
                    let params = (&el_count, &dims_len, &ds, &src, &mut dst);
                    // SAFETY: ffi.
                    dev.launch_on_stream(&stream, &func, cfg, params).w()?
                }
            }
