...
fn quantize_q8_1(
    ...
    let params = (src, dst, &kx_i32, &kx_padded_i32);
...
fn dequantize_f32(
    ...
    if is_k {
        ...
        let params = (&data.inner, &dst, &nb32_i32);
        dev.launch_on_stream(&stream, &func, cfg, params).w()?;
    }
...
fn dequantize_mul_mat_vec(
    ...
    let params = (&data.inner, y, &dst, &ncols_i32, &nrows_i32);
    let stream = dev.launch_stream();
    dev.launch_on_stream(&stream, &func, cfg, params).w()?;
    Ok(CudaStorage::wrap_cuda_slice(dst, dev.clone()))
}
...
fn mul_mat_via_q8_1(
    ...
    let params = (
        ...
        /* nrows_x */ &x_rows_i32,
        /* ncols_y */ &y_cols_i32,
        /* nrows_y */ &k_padded_i32,
        /* nrows_dst */ &x_rows_i32,
    );
    let stream = dev.launch_stream();
    dev.launch_on_stream(&stream, &func, cfg, params).w()?;
    Ok(CudaStorage::wrap_cuda_slice(dst, dev.clone()))
