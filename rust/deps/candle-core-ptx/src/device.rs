...
impl Device {
    ...
    #[cfg(feature = "ptx-os")]
    /// Create a CUDA device using a PTX-OS stream pool.
    ///
    /// # Safety
    /// `runtime_ptr` must be a valid PTX-OS GPUHotRuntime pointer that outlives the device.
    pub unsafe fn new_cuda_with_ptx_streams(
        ordinal: usize,
        runtime_ptr: *mut std::ffi::c_void,
        num_streams: usize,
    ) -> Result<Self> {
        Ok(Self::Cuda(crate::CudaDevice::new_with_ptx_streams(
            ordinal,
            runtime_ptr,
            num_streams,
        )?))
