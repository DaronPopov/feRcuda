...
fn run_candle_workload() -> Result<()> {
    let device = Device::new_cuda(0)?;
    let a = Tensor::randn(0f32, 1.0, (1024, 1024), &device)?.to_dtype(DType::F32)?;
    let b = Tensor::randn(0f32, 1.0, (1024, 1024), &device)?.to_dtype(DType::F32)?;
    let c = a.matmul(&b)?;
...
fn main() -> Result<()> {
    ...
    println!("fercuda intercept stats:");
    ...
    println!("pattern check: PASS (Candle process exercised fercuda malloc interception)");
