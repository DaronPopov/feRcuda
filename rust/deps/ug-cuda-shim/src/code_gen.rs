...
pub fn gen<W: std::io::Write>(w: &mut W, func_name: &str, kernel: &ssa::Kernel) -> Result<()> {
    ...
    for (var_id, instr) in instrs.iter().enumerate() {
        ...
        match instr {
            ...
            I::Special(ssa::Special::ThreadIdx) => writeln!(w, "{indent}int {var_id} = threadIdx.x;")?,
            I::Special(ssa::Special::BlockIdx) => writeln!(w, "{indent}int {var_id} = blockIdx.x;")?,
            I::Barrier => writeln!(w, "{indent}__syncthreads();")?,
            I::ReduceLocal { .. } => {
                ug::bail!("reduce-local kernels are not supported by the local ug-cuda shim")
            }
        }
    }
    writeln!(w, "}}")?;
