...
#[cfg(feature = "ptx-os")]
fn link_ptx_os_runtime() {
    use std::env;
    use std::path::PathBuf;

    let manifest_dir = env::var("CARGO_MANIFEST_DIR").unwrap();
    // Walk up to feRcuda root: deps/cudarc-ptx -> deps -> rust -> feRcuda
    let fercuda_root = PathBuf::from(&manifest_dir)
        .parent().unwrap()
        .parent().unwrap()
        .parent().unwrap()
        .to_path_buf();
    let build_dir = env::var("FERCUDA_BUILD_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| fercuda_root.join("build"));
    ...
    println!("cargo:rerun-if-changed={}/libptx_core.a", build_dir.display());
    println!("cargo:rerun-if-changed={}/libptx_kernels.a", build_dir.display());
}
