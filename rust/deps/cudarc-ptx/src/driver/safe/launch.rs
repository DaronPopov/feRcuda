...
use super::{
    CudaEvent, CudaFunction, CudaModule, CudaSlice, CudaStream, CudaView, CudaViewMut, DeviceRepr,
};
...
pub struct LaunchArgs<'a> {
    pub(super) stream: std::sync::Arc<CudaStream>,
    pub(super) cu_function: sys::CUfunction,
    pub(super) _module: std::sync::Arc<CudaModule>,
    pub(super) waits: Vec<&'a CudaEvent>,
    pub(super) records: Vec<&'a CudaEvent>,
    pub(super) args: Vec<*mut std::ffi::c_void>,
    pub(super) owned_args: Vec<Box<dyn Any>>,
...
impl std::fmt::Debug for LaunchArgs<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("LaunchArgs")
            .field("stream", &"<cuda stream>")
            .field("func", &"<cuda function>")
            .field("waits", &self.waits.len())
            .field("records", &self.records.len())
            .field("args", &self.args.len())
            .field("owned_args", &self.owned_args.len())
...
impl CudaStream {
    ...
    pub fn launch_builder<'a>(self: &std::sync::Arc<Self>, func: &CudaFunction) -> LaunchArgs<'a> {
        LaunchArgs {
            stream: self.clone(),
            cu_function: func.cu_function,
            _module: func.module.clone(),
            waits: Vec::new(),
            records: Vec::new(),
            args: Vec::new(),
            owned_args: Vec::new(),
...
macro_rules! impl_value_kernel_arg {
    ($($t:ty),+) => {
        $(unsafe impl<'a> PushKernelArg<$t> for LaunchArgs<'a> {
            #[inline(always)]
            fn arg(&mut self, arg: $t) -> &mut Self {
                let boxed = Box::new(arg);
                let ptr = (&*boxed) as *const $t as *mut _;
                self.args.push(ptr);
                self.owned_args.push(boxed);
                self
            }
        })+
    };
...
pub trait LaunchArgsTuple<'a> {
    fn push(self, args: &mut LaunchArgs<'a>);
}

impl<'a> LaunchArgsTuple<'a> for () {
    fn push(self, _args: &mut LaunchArgs<'a>) {}
}

macro_rules! impl_launch_args_tuple {
    ($($name:ident),+) => {
        impl<'a, $($name),+> LaunchArgsTuple<'a> for ($($name,)+)
        where
...
impl_launch_args_tuple!(A);
impl_launch_args_tuple!(A, B);
impl_launch_args_tuple!(A, B, C);
impl_launch_args_tuple!(A, B, C, D);
impl_launch_args_tuple!(A, B, C, D, E);
impl_launch_args_tuple!(A, B, C, D, E, F);
impl_launch_args_tuple!(A, B, C, D, E, F, G);
impl_launch_args_tuple!(A, B, C, D, E, F, G, H);
impl_launch_args_tuple!(A, B, C, D, E, F, G, H, I);
impl_launch_args_tuple!(A, B, C, D, E, F, G, H, I, J);
impl_launch_args_tuple!(A, B, C, D, E, F, G, H, I, J, K);
impl_launch_args_tuple!(A, B, C, D, E, F, G, H, I, J, K, L);

pub unsafe trait LaunchAsync {
    unsafe fn launch<'a, A>(&self, cfg: LaunchConfig, args: A) -> Result<(), DriverError>
    where
        A: LaunchArgsTuple<'a>;
}

unsafe impl LaunchAsync for CudaFunction {
    unsafe fn launch<'a, A>(&self, cfg: LaunchConfig, args: A) -> Result<(), DriverError>
    where
        A: LaunchArgsTuple<'a>,
    {
        let stream = self.module.ctx.default_stream();
        let mut builder = stream.launch_builder(self);
