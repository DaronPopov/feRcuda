...
#[inline]
fn use_async_alloc(ctx: &CudaContext) -> bool {
    ctx.has_async_alloc && !result::ptx_os_hooks_enabled()
}
...
pub struct CudaStream {
    ...
    pub(crate) ctx: Arc<CudaContext>,
    /// If true, this stream is externally owned (e.g., from PTX-OS) and should not be destroyed on drop.
    pub(crate) external: bool,
}

impl PartialEq for CudaStream {
    fn eq(&self, other: &Self) -> bool {
        self.cu_stream == other.cu_stream && Arc::ptr_eq(&self.ctx, &other.ctx)
    }
}
...
impl Drop for CudaStream {
    fn drop(&mut self) {
        // Don't destroy external streams (e.g., from PTX-OS)
        if self.external {
            return;
        }
        self.ctx.record_err(self.ctx.bind_to_thread());
...
impl CudaContext {
    ...
    /// Wrap an external stream handle (e.g., from PTX-OS).
    ///
    /// The stream will NOT be destroyed when the wrapper is dropped.
    /// This is useful for integrating with external stream pools like PTX-OS KernelAccelerator.
    ///
    /// # Safety
    /// The caller must ensure:
    /// - The stream handle is valid and was created on the same device as this context
    /// - The stream will outlive this wrapper
    pub unsafe fn wrap_external_stream(self: &Arc<Self>, cu_stream: sys::CUstream) -> Arc<CudaStream> {
        Arc::new(CudaStream {
            cu_stream,
            ctx: self.clone(),
            external: true,
...
impl CudaStream {
    ...
    /// Returns true if this stream is externally owned (e.g., from PTX-OS).
...
impl CudaStream {
    ...
    pub fn null<T>(self: &Arc<Self>) -> Result<CudaSlice<T>, result::DriverError> {
        self.ctx.bind_to_thread()?;
        let alloc_async = use_async_alloc(&self.ctx);
        let cu_device_ptr = if alloc_async {
